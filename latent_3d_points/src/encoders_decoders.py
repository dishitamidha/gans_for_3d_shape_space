...
def encoder_with_convs_and_symmetry(in_signal, n_filters=[64, 128, 256, 1024], filter_sizes=[1], strides=[1],
    ...
    for i in range(n_layers):
        ...
        if verbose:
            print(name, 'conv params = {}'.format(np.prod(layer.W.get_shape().as_list()) + np.prod(layer.b.get_shape().as_list())), end='\n')
        ...
        if b_norm:
            ...
            if verbose:
                print('bnorm params = {}'.format(np.prod(layer.beta.get_shape().as_list()) + np.prod(layer.gamma.get_shape().as_list())), end='\n')
        ...
        if verbose:
            print (layer)
            print('output size:', np.prod(layer.get_shape().as_list()[1:]), end='\n')
